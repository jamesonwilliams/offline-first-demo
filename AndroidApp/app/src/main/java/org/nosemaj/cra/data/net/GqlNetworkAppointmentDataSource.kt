...
class GqlNetworkAppointmentDataSource @Inject constructor(
    private val apolloClient: ApolloClient,
) : NetworkAppointmentDataSource {
    override suspend fun updateAppointment(
        appointment: AppointmentModel,
    ): Result<AppointmentModel> = runCatching {
        val mutation = with(appointment) {
    ...
    private fun monitorAppointmentChanges(): Flow<AppointmentModel> {
        return apolloClient
            .subscription(AppointmentChangeSubscription())
            .toFlow()
            .map { data ->
                data
                    .dataAssertNoErrors
    ...
    private fun AppointmentModel.toAppointmentInput(): AppointmentInput = AppointmentInput(
        id = id.toString(),
